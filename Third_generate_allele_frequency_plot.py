...
def plot_data(output,sample_name):
	...
	n, bins, patches=plt.hist(af_numbers,bins=31,range=(-0.01,1.01),edgecolor='steelblue',color='steelblue',density='true')
		...
		if snp_increament==number_of_snps or chr_first!=line[0]:
			if depth==0:
				...
				h1_af=float(float(n_h1)/float(depth))
                        out="\t".join([chr_first,pos_first+"-"+pos_end,str(snp_increament),str(h1_af),str(n_h1),str(n_h2)])
			output_fh.write(out+"\n")
                        snp_increament=0
                        n_h1=0
                        n_h2=0
