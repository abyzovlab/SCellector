...
def argument_parse(script_path):
	...
	parser.add_argument("-f","--fast_option",help="run all chromosome in parallel.Uses more memory ",action='store_true')
	return parser
	...
	if chromosome_number==None:
		...
		for chromosome_number in list_of_chromosomes:
			if all_chr_at_one==False:
				...
				t=threading.Thread(target=running_preprocessing,args=(vcf,output_dir,sample_name,chromosome_number,g1k_snp_dict))
				time.sleep(5)
				t.start()
				time.sleep(5)
